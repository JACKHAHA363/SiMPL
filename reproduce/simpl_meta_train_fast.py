...
def simpl_meta_train_iter(
    conc_collector, trainer, train_task_ids, train_tasks, *,
    ...
    for high_episode, task_id in zip(prior_high_episodes, train_task_ids):
        enc_buffer = trainer.enc_buffers[task_id]
        buffer = trainer.buffers[task_id]
    ...
    for high_episode, task_id in zip(post_high_episodes, train_task_ids):
        buffer = trainer.buffers['task_id']
        buffer.enqueue(high_episode)
    ...
    log.update({
        ...
        'tr_return': {task_idx: tr_return for task_idx, tr_return in zip(train_task_ids,tr_returns)}
    })
...
if __name__ == '__main__':
    ...
    parser.add_argument('-N', '--max_train_tasks', default=10, help='Maximum number of training tasks per iteration')
    ...
    wandb_run_name = args.wandb_run_name or args.domain + '.simpl_meta_train_fast.' + wandb.util.generate_id()
